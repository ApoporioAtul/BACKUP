    ...
    */
    public func dictionaryRepresentation() -> [String : AnyObject ] {
		...
		if originAddresses?.count > 0 {
			dictionary.updateValue(originAddresses!, forKey: kDistanceModelOriginAddressesKey)
		}
		if rows?.count > 0 {
			var temp: [AnyObject] = []
			for item in rows! {
			...
			dictionary.updateValue(temp, forKey: kDistanceModelRowsKey)
		}
		if destinationAddresses?.count > 0 {
			dictionary.updateValue(destinationAddresses!, forKey: kDistanceModelDestinationAddressesKey)
		}
		if status != nil {
			dictionary.updateValue(status!, forKey: kDistanceModelStatusKey)
		}
